<template>
    ...
    >
        ...
        >
            <span class="el-tiptap-editor__characters"> {{ t('editor.characters') }}: {{ characters }} </span>
        </div>
    </div>
</template>
        ...
        extensions.map((extension) => {
            if (extension?.parent?.config?.nessesaryExtensions || extension?.config?.nessesaryExtensions) {
                additionalExtensions = [
                    ...additionalExtensions,
                    ...(extension?.parent?.config?.nessesaryExtensions || extension?.config?.nessesaryExtensions),
                ]
            }
        })
        const uniqueObjects = []
